let renderer2, scene2, camera2, cameraCtrl;
...
function init() {
  renderer2 = new THREE.WebGLRenderer({ canvas: document.getElementById('reveal-effect'), antialias: true, alpha: true });
  renderer2.setSize(window.innerWidth, window.innerHeight);
  document.body.appendChild(renderer2.domElement);

  camera2 = new THREE.PerspectiveCamera(conf.perspective, window.innerWidth / window.innerHeight, 0.1, 1000);
  camera2.position.z = conf.cameraZ;
...
function initLights() {
  scene2.add(new THREE.AmbientLight(conf.ambientColor));
...
function getRendererSize() {
  const cam = new THREE.PerspectiveCamera(conf.perspective, camera2.aspect);
